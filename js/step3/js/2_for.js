...
function answer1(){
    for (var i = 0; i < 10; i++){ //? 10보다 작을때 (0 ~ 9)까지 반복수행
        document.write((i + 1) + '<br>'); //? i+=1 1부터 10까지 출력 
    }    
}
function answer2(){
...
function answer3(){
    for (var i = 100; i < 110; i++){ //? 110보다 작거나 같을때 100 - 109까지 반복수행
        document.write((i - 99) + '<br>'); //? 100..(-99) 1 ~ 10번까지 출력
    }
}
function answer4(){
    for (var i = 1; i<=10; i+=2){ //? 10보다 작거나 같을때 1 - 10까지 반복수행. 반복내 수행이 끝난후 i에 2를 더함.
        document.write(i + "<br>"); //? 1.3.5.7.9... 홀수 출력
        document.write((i+1) + "<br>"); //? 2.4.6.8.10... 짝수 출력
    }
}
function answer5(){
    for (var i = 10; i >= 1; i--){ //? 1보다 크거나 같을 때 10 - 1 까지 반복수행
...
function favorFluit(){
    var array = ['딸기','사과','귤','복숭아'];
    
    for (var i = 0; i < array.length; i++){
        // ? length는 배열의 갯수를 센다 array.length = 4
        alert(array[i])
    }
}
...
function reverseFavorFluit(){
    ...
    // todo. 역반복걸어보기
    for (var i = array.length - 1; i >= 0; i--){ //? 변수는 배열의 수 - 1 (4 - 1 = 3)
        // ? 역순이라고 변수 i에 3을 주면 길이는 끝을 지정할수 없기에 
        // ?  먼저 length를 쓰고 데이터 배열에는 4가 없기에 - 1 를 해준다
        document.write(array[i]); //? 3,2,1,0 순으로 출력.
    }
}
...
function runBreak(){
    for (var i = 0; true; i++){
       ...
       //? 조건충족을 일부러 안시켜서 반복문이 되게끔하는것 논리부정을 고의로 해서 break가 안걸리게끔 한다
       //? ex ) 긍정적인 질문에는 논리부정.
