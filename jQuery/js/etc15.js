...
function updateFishPosition(){
    ...
    for(var i = 0; i < $fishList.length; i++){
        var $fish = $fishList.eq(i); //? 루프당 개별 물고기 (ex> i = 0 -> 0번 물고기)
...
function displayFishPositionInfo(){
    ...
    for(var i =0; i < $fishList.length; i++){ //? 물고기 마릿수만큼 반복
        var $fish = $fishList.eq(i); //? 각 i번쨰 몰고기 선택
...
function checkGoalFish(){
    ...
    if(winnerList.length > 0){ //? 배열에 position 값이 담기고 나면 
        winnerList.sort(function(a,b){ //? a(index , position) 이 딸려옴
            return b.position-a.position; //? position 값 기준 내림차순으로 정렬 b-a 공식
        });
    }
}